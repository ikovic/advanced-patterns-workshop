...
const makeTypeSafeGet =
  <QueryP extends Request["query"]>(
    parser: (queryParams: Request["query"]) => QueryP,
    handler: RequestHandler<any, any, any, QueryP>
  ) =>
  (req: Request<any, any, any, QueryP>, res: Response, next: NextFunction) => {
    try {
      parser(req.query);
    } catch (e) {
