...
class DynamicMiddleware<TInput, TOutput> {
  ...
  // whenever we add middleware, we are piping the output of previous middleware to the input of the next one
  // also we don't know what the output type is
  // on the top level this returns the DynamiMiddleware instance with TInput (does not change) and the new output type (based on the last middleware added)
  // the last middleware receives the output of the previous middleware as Input (so TOutput is used as first generic argument)
  // it produces the NewOutput which then becomes the output type of DynamicMiddleware
  use<NewTOutput>(
